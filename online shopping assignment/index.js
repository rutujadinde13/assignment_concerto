...
function displayItems() {
    var inputitem = document.getElementById('category').value;
    console.log(inputitem);
    displayItem(inputitem)
}
/**
 ...
 */
function displayItem(items_) {

    switch (items_) {
        case 'biryani':
            var itemfound = foodItem.filter(function (category) { return category.category == items_ })
            console.log(itemfound)
            createHTML(itemfound)
            break;
        case 'chicken':
            var itemfound = foodItem.filter(function (category) { return category.category == items_ })
            console.log(itemfound)
...
function createHTML(itemfound) {
    console.log(itemfound)
    var itemdisplay = document.getElementById('items');
    var html = ''
    itemdisplay.innerHTML = ''
    for (let j = 0; j < itemfound.length; j++) {
        
            html += 
            `<div class="row text-center m-b-5">
               <div class="col-md-3 text-center">
                 <h2><img src='${itemfound[j].img}' width=150 height=200/></h2>
                </div>
                <div class="col-md-2">
                 <h2>${itemfound[j].name}</h2>
                </div>
                <div class="col-md-2">
                 <h2>${itemfound[j].price}</h2>
                </div>
                <div class="col-md-2">
                 <h2>${itemfound[j].rating}</h2>
                </div>
                <div class="col-md-2">
                 <h3><input type="checkbox" id='ch${itemfound[j].id}' onClick="addToCart(${itemfound[j].id})"></h3>
                </div>
            </div>
            `;
    }
...
function addToCart(itemid) {
    ...
    let addItem=foodItem.find(function(item_id){return item_id.id==itemid;})
    console.log(addItem);
    ...
    // ele.checked = true;
        let pushItem = cartData.find(i_id=>i_id.id==addItem.id)
        console.log(pushItem)
 
        if((ele.checked==true)&&(addItem!=pushItem))
        {
            ...
            var alreadyadd="ch"+itemid;
            document.getElementById(alreadyadd).checked=true;
            document.getElementById(alreadyadd).disabled=true;
